...
def get_basic_info(cursor):
    ...
    queries = {
        ...
        "Total Sale Value (Last 3 Months)": """
            SELECT ROUND(COALESCE(SUM(ABS(se.change_quantity) * p.price), 0), 2) AS sales_amount 
            FROM stock_entries AS se
            JOIN products AS p
            ON p.product_id = se.product_id
            WHERE se.change_type = 'Sale'
            AND se.entry_date <= DATE_SUB(
            (SELECT MAX(entry_date) FROM stock_entries),
            INTERVAL 3 MONTH);""",

        "Total Restock Value (Last 3 Months)": """
            SELECT ROUND(SUM(se.change_quantity * p.price), 0) AS total_restock
...
def get_additonal_tables(cursor):
    queries = {
        ...
        "Products Needing Reorder": """
            SELECT product_name, stock_quantity, abs(stock_quantity - reorder_level ) ordering_qty
            FROM products
...
#----------------------- Product Inventory Dashboard Page ---------------------
