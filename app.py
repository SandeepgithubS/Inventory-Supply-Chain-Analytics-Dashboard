...
import pandas as pd
from db_function import (connection, get_basic_info, get_additonal_tables, add_manual_id, get_categories, get_suppliers,
    get_product_history, get_all_products, place_reorder, get_pending_reorders, mark_reorder_as_received)

# Side bar
st.sidebar.title("Inventory Management Dashboard")
option= st.sidebar.radio("Select Options:", ["Basic Info.","Operational Task"])
...
if option == "Basic Info.":
    ...
    # Fetch and disply tables for additional details
...
#----------------------- OPERATIONAL TASK PAGE ---------------------
...
elif st.header("Operational Tasks"):
    selected_task= st.selectbox("Select Operational Task:", ["Add new Product","Product History","Place Reorder","Receive Reorder"])
    ...
    if selected_task== "Add new Product":
        ...
        with st.form("Add New Products"):
            ...
            supplier_id=st.selectbox("Suppliers", options= supplier_ids,
            ...
            if submitted:
                if not product_name:
                    ...
                    try:
                        add_manual_id(cursor, db, product_name, product_category, product_price, product_stock, product_level, supplier_id)
